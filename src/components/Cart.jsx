...
function Cart({ items, setCart, data, updateCart }) {
  const [confirmed, setConfirmed] = useState(false);

  const names = Object.keys(items);

  const totalCount = names.reduce((acc, name) => acc + items[name], 0);

  const orderTotal = names.reduce((acc, name) => {
        ...
        <div className="grid h-full p-4 bg-rose-50 rounded-xl">
          ...
          {names.length === 0 ? (
            <div className="flex flex-col justify-between">
              <svg
                className="mx-auto my-5 self-center"
            ...
            <div>
              <div>
                {names.map((name) => {
                  ...
                  return (
                    <div
                      key={name}
                      className="flex justify-between items-center border-b border-b-rose-300 py-4"
                    >
                      <div>
                        <h3 className="font-medium mb-2">{name}</h3>
                        <span className="quantity text-red font-bold mr-2">
                          {count}x
                        </span>
                        <span className="product-price text-rose-400 mr-1">
                          @${product.price.toFixed(2)}
                        </span>
                        <span className="total-order text-rose-500">
                      ...
                      <svg
                        ...
                        tabIndex="0"
                        className="text-rose-300 rounded-full border border-rose-300 p-[3px] cursor-pointer hover:text-rose-900 hover:border-rose-900 transition duration-300"
                        xmlns="http://www.w3.org/2000/svg"
                        width="20"
                        height="20"
                        fill="none"
              ...
              <div className="bg-rose-100 flex items-center justify-center p-3 my-5 rounded-lg">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="21"
                  height="20"
              ...
              <button
                className="bg-red text-white p-4 w-full rounded-4xl cursor-pointer hover:bg-red-800 transition duration-300"
