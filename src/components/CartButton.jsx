function CartButton({ name, onChange, count }) {
  const addToCart = () => {
    onChange(name, 1);
  };

  const increase = () => {
    onChange(name, count + 1);
  };

  const decrease = () => {
    onChange(name, count - 1 <= 0 ? 0 : count - 1);
  };

  return (
    <div>
      {count > 0 ? (
        <button
          tabIndex="-1"
          className="h-10 w-36 flex items-center justify-between bg-red text-white mx-auto -m-5 px-4 rounded-3xl relative z-10 focus:outline-red focus:outline-offset-3 transition-colors duration-300"
        >
          <svg
            data-testid="minus"
            onClick={decrease}
            tabIndex="0"
            className="rounded-full border border-white p-1 cursor-pointer hover:text-red hover:bg-white focus:outline-transparent transition-colors duration-300"
            xmlns="http://www.w3.org/2000/svg"
            width="20"
            height="20"
            fill="none"
            viewBox="0 0 10 2"
          >
            <path fill="currentColor" d="M0 .375h10v1.25H0V.375Z" />
          </svg>
          <span>{count}</span>
          <svg
            data-testid="plus"
            onClick={increase}
            tabIndex="0"
            className="rounded-full border border-white p-1 cursor-pointer hover:text-red hover:bg-white focus:outline-transparent transition-colors duration-300"
            xmlns="http://www.w3.org/2000/svg"
            width="20"
            height="20"
            fill="none"
            viewBox="0 0 10 10"
          >
            <path
              fill="currentColor"
              d="M10 4.375H5.625V0h-1.25v4.375H0v1.25h4.375V10h1.25V5.625H10v-1.25Z"
            />
          </svg>
        </button>
      ) : (
        <button
          className="h-10 w-36 flex items-center justify-center gap-3 bg-rose-50 mx-auto -m-5 rounded-3xl border border-rose-500 cursor-pointer relative z-10 hover:text-red hover:border-red focus:outline-red focus:outline-offset-3 transition-colors duration-300"
          onClick={addToCart}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="21"
            height="20"
            fill="none"
            viewBox="0 0 21 20"
          >
            <g fill="#C73B0F" clipPath="url(#a)">
              <path d="M6.583 18.75a1.25 1.25 0 1 0 0-2.5 1.25 1.25 0 0 0 0 2.5ZM15.334 18.75a1.25 1.25 0 1 0 0-2.5 1.25 1.25 0 0 0 0 2.5ZM3.446 1.752a.625.625 0 0 0-.613-.502h-2.5V2.5h1.988l2.4 11.998a.625.625 0 0 0 .612.502h11.25v-1.25H5.847l-.5-2.5h11.238a.625.625 0 0 0 .61-.49l1.417-6.385h-1.28L16.083 10H5.096l-1.65-8.248Z" />
              <path d="M11.584 3.75v-2.5h-1.25v2.5h-2.5V5h2.5v2.5h1.25V5h2.5V3.75h-2.5Z" />
            </g>
            <defs>
              <clipPath id="a">
                <path fill="#fff" d="M.333 0h20v20h-20z" />
              </clipPath>
            </defs>
          </svg>
          Add to Cart
        </button>
      )}
    </div>
  );
}

export default CartButton;
